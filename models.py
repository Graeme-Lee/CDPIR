...
class SiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # # 修改这里以同时得到attention权重
        # x_norm = modulate(self.norm1(x), shift_msa, scale_msa)
        # B, N, C = x_norm.shape

        # # Attention模块需要return_attn=True
        # attn_output, attn_map = self.attn(x_norm, return_attn=True)
        # self.last_attn_map = attn_map.detach().cpu()  # 保存attention map
